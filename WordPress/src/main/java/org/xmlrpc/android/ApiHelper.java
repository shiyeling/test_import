...
public class ApiHelper {
    ...
    /**
     * New task to fetch posts by categories. Need four parameters:
     * 1. the Blog object for the remote blog*
     * 2. the category id*
     * 3. number of posts to skip, set to 0 when doing initial load or complete refresh, which will clean up all cached posts*
     * 4. number of posts to fetch, *
     */
    public static class FetchPostsByCategoriesTask extends HelperAsyncTask<java.util.List<?>, Boolean, Boolean> {
        public interface Callback extends GenericErrorCallback {
            public void onSuccess(int postCount);
        }
        ...
        public FetchPostsByCategoriesTask(Callback callback) {
            mCallback = callback;
        }
        ...
        protected Boolean doInBackground(List<?>... params) {
            ...
            Object[] result;
            int categoryId = (Integer) arguments.get(1);
            int offset = (Integer) arguments.get(2);
            int numberPosts = (Integer) arguments.get(3);
            boolean loadMore = offset > 0; //offset bigger than 0 implies loading more mode.
            Object[] xmlrpcParams = {
                    blog.getRemoteBlogId(),
                    blog.getUsername(),
                    blog.getPassword(), categoryId, offset, numberPosts

            };
            try {
                result = (Object[]) client.call("metaWeblog.getRecentPostsByCategory", xmlrpcParams);
                if (result != null && result.length > 0) {
                    mPostCount = result.length;
                    List<Map<?, ?>> postsList = new ArrayList<Map<?, ?>>();

                    if (!loadMore) {
                        WordPress.wpDB.deleteUploadedPostsByCategory( blog.getLocalTableBlogId(), categoryId, false);
                    }
                    ...
                    WordPress.wpDB.savePosts(postsList, blog.getLocalTableBlogId(), false, !loadMore);
                }
                return true;
            } catch (XMLRPCFault e) {
                mErrorType = ErrorType.NETWORK_XMLRPC;
